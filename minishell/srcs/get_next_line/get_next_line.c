...
/*   Updated: 2021/04/28 20:47:32 by hyungyyo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		get_line(char **remain_str, char **line, int cut_pos)
{
	...
	*line = ft_gstrdup(*remain_str);
	len = ft_gstrlen(*remain_str + cut_pos + 1);
	if (len == 0)
	{
		free(*remain_str);
		*remain_str = 0;
	...
	tmp = ft_gstrdup(*remain_str + cut_pos + 1);
	free(*remain_str);
	*remain_str = tmp;
	return (RET_READED);
}
